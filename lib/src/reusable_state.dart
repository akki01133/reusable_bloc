...
abstract class DataInitialized<T>  extends DataState<T>{
  ...
  DataInitialized.clone(DataInitialized<T>  oldState) : this(data: oldState.data);
...
class DataRefetchingError<T> extends DataInitialized<T>  with DataError<T> {
  final dynamic error;
  DataRefetchingError(DataInitialized<T>  oldState, this.error) : super.clone(oldState);
